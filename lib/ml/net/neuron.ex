defmodule Ml.Net.Neuron do
  ...
  defmodule State do
    @moduledoc false
    defstruct [:ws, :xpids_with_vals, :ypids_with_ds, :a]
  end
  ...
  def start_link(num_of_inputs, learning_rate, ys_ids) do
    ...
    state = %State{ws: ws, xpids_with_vals: xpids_with_vals, ypids_with_ds: ypids_with_ds, a: a}
    Task.start_link(fn -> loop(state) end)
  end

  defp forward(%State{ws: ws, xpids_with_vals: xpids_with_vals, ypids_with_ds: ypids_with_ds}) do
    y = xpids_with_vals
  ...
  defp backward(%State{ws: ws, xpids_with_vals: xpids_with_vals, ypids_with_ds: ypids_with_ds, a: a}) do
    ...
    loop(%State{ws: new_ws, xpids_with_vals: new_xpids_with_vals, ypids_with_ds: new_ypids_with_ds, a: a})
  end

  defp maybe_act(%State{ws: ws, xpids_with_vals: xpids_with_vals, ypids_with_ds: ypids_with_ds} = state) do
    cond do
      Enum.all?(ypids_with_ds, fn {_, d} -> d != nil end) -> backward(state)
      Enum.count(xpids_with_vals) == length(ws) -> forward(state)
      true -> nil
    end
  end

  defp loop(%State{ws: ws, xpids_with_vals: xpids_with_vals, ypids_with_ds: ypids_with_ds, a: a} = state) do
    maybe_act(state)
    receive do
      {:x, x, caller} ->
        ...
        loop(%State{ws: ws, xpids_with_vals: new_xpids_with_vals, ypids_with_ds: ypids_with_ds, a: a})
      {:y, d, caller} ->
        new_ypids_with_ds = Utils.update_keyword_list(ypids_with_ds, caller, d)
        loop(%State{ws: ws, xpids_with_vals: xpids_with_vals, ypids_with_ds: new_ypids_with_ds, a: a})
    end
  end
